# 班牌专用输入法（基于语燕输入法 YuyanIme 二次开发）

> 本仓库是 [gurecn/YuyanIme](https://github.com/gurecn/YuyanIme) 的二次开发版本，目标：**电子智能班牌专用的横屏、大触控区域、轻量稳定的专用输入法**。完整需求见 `PROJECT-HANDOFF.md`。

## 本版本改了什么

- **Bar 按钮**：工具栏常驻。隐藏输入法并尽力恢复系统导航栏（ROM 不配合时安全降级，不崩溃）。
- **HOME 按钮**：工具栏常驻。隐藏输入法并尝试回到 Android 桌面（不杀死其他应用）。
- **横屏三栏布局**：左侧方案选择（26键全拼/双拼/9键拼音，高亮当前）、右侧数字小键盘+编辑键（方向/Home/End/Del/Ctrl+A/C/X/V）；竖屏、悬浮、窄屏自动回退原布局。
- **公共设备化**：每日自动清理 Rime userdb 与引擎日志，关闭自学习数据的无限增长，多人共用体验统一。
- 保留 YuyanIme 的 Rime 引擎、中文方案、候选词、上屏逻辑与构建体系，未重写任何输入核心。

## 构建方式（本地零安卓环境）

本地 ZCode 写码 → push 到 GitHub → GitHub Actions 云端构建 → 从 Actions Artifacts 下载 `app-debug-apk` 侧载安装。

```
./gradlew assembleOfflineDebug   # CI 自动执行；产物在 app/build/outputs/apk/offline/debug/
```

CI 失败时会把构建日志关键行发布到对应 commit 的 comment 里，便于离线诊断。
